...
def plot_model(region, fn):
    ...
    _gmt_fig_model(topo, tomos, fn, sta=None)
...
def plot_area_map(regions, fig_name):
    ...
    gmt_fig_area(vicinity, area, fig_name, eles)


def gmt_fig_area(vici, area, fn, eles, line=None):
    fig = pygmt.Figure()
    pygmt.config(
        MAP_FRAME_TYPE="plain",
        MAP_TITLE_OFFSET="0.25p",
    ...
    with fig.subplot(
        ...
        kwgs = {"projection": "M?"}
        with fig.set_panel(panel=0):
            fig = fig_tomos(fig, **vici, **kwgs, **eles)
            fig.text(
        ...
        with fig.set_panel(panel=1):
            ...
            eles |= {"tect": 2, "tect_pen": "0.3p,black,-"}
            fig = fig_tomos(fig, area, [], **kwgs, **eles)
            fig.text(
