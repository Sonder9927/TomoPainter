...
def plot_model(region, fn):
    ...
    tomos = _sed_moho(region)
    _gmt_fig_model(topo, tomos, fn, sta)


def _sed_moho(region):
    tomos = [
    ...
    makecpt([27, 35, 0.1], tomos[1]["cmap"])
    return tomos


def _gmt_fig_model(topo, tomos, fn, sta):
    fig = pygmt.Figure()
    pygmt.config(
    ...
    with fig.subplot(
        ...
        kwgs = {"projection": "M?", "tect": 0, "sta": sta, "clip": True}
...
def _gmt_fig_area(vici, area, fn, sta=None, line=None):
    fig = pygmt.Figure()
    pygmt.config(
        MAP_FRAME_TYPE="plain",
        MAP_TITLE_OFFSET="0.25p",
    ...
    with fig.subplot(
        ...
        with fig.set_panel(panel=1):
            if line:
                lines = [
                    ll
                    for lt in ["xline", "yline", "arise", "decline"]
                    for _, ll in lines_generator(area["region"], lt)
                ]
                lines = np.array(lines)
                kwgs |= {"lines": [], "line_pen": "red"}
            kwgs |= {"tect": 2, "tect_pen": "0.3p,black,-"}
...
def plot_misfit(grid, region, fig_name) -> None:
    # make cpt file
    cptfile = makecpt([-0, 2, 0.05], cmap="hot", reverse=True)
